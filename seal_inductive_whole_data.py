...
class SEALDataset(InMemoryDataset):
    def __init__(self, state, dataset, root, data, split_edge, num_hops, percent=100, split='train', 
    ...
    def process(self):
        path_edges = "./datasets_LLGF/edges/"+self.dataset
        if not '_all' in path_edges:
            path_edges = path_edges[:-3]+'_all'
        if os.path.isfile(path_edges+'/pos_'+self.state+'.pt'):
            print("load data")
            pos_edge = torch.load(path_edges+'/pos_'+self.state+'.pt')
            neg_edge = torch.load(path_edges+'/neg_'+self.state+'.pt')
        else:
            print("extract and save")
            ...
            if self.state == 'train':
                os.mkdir(path_edges)
            torch.save(pos_edge, path_edges+'/pos_'+self.state+'.pt')
            torch.save(neg_edge, path_edges+'/neg_'+self.state+'.pt')
...
parser.add_argument('--dataset', type=str, default='LLGF_cora_new_all')
...
for run in range(args.runs):
    ...
    if args.only_test:
        ...
        if 'ind' in args.dataset:
            ds = args.dataset[:-3]
        checkpoint = torch.load('/localhome/pnaddaf/Desktop/parmis/SEAl_miror/model' + ds + '.pth')
