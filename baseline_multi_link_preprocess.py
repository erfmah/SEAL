...
# -*- coding: utf-8 -*-
"""
Created on Wed May  4 15:05:04 2022
...
def make_semi_inductive_edges(adjacency_matrix, inductive_test_edges, inductive_val_edges):
    ...
    return np.array(list(set(map(tuple, edge_list)) - set(map(tuple, inductive_val_edges))))
...
def get_test_nodes(test_edges_pos, test_edges_false):
    nodes = set()
    for e in test_edges_pos:
        nodes.add(e[0])
        nodes.add(e[1])
...
def create_adj(feat_data, test_edges):

    test_edges = torch.tensor([test_edges[:,0], test_edges[:,1]])
    adj = to_scipy_sparse_matrix(test_edges)
...
def return_trans_ind(feat_data, adjacency_matrix, output_dataset):
    ...
    test_nodes = get_test_nodes(inductive_test_edges, inductive_test_edges_false)
    test_edges = np.concatenate([np.array(inductive_test_edges_false),np.array(inductive_test_edges_false)])
    ...
    for i in test_nodes:
        if np.count_nonzero(adj[i])>1 and counter<100:
            ...
            neg_test_nodes = neg_test_nodes[:len(pos_test_nodes)]
            i_node = np.array([i] * len(pos_test_nodes)) 
            ...
            np.save(save_path + 'LLGF_' + output_dataset +  '_ind_' + str(counter) +'_val_pos.npy', np.array(inductive_valid_edges))
            np.save(save_path + 'LLGF_' + output_dataset + '_ind_' + str(counter) +'_val_neg.npy', np.array(inductive_val_edges_false))
