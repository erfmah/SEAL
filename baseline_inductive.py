...
def evaluate_auc(val_pred, val_true, test_pred, test_true):
    ...
    if "_semi_" in args.dataset:
        with open('results_semi.csv', 'a', newline="\n") as f:
            writer = csv.writer(f)
            writer.writerow([args.dataset[5:]])
            writer.writerow([precision_test, recall_test, acc_val, ap_test, test_auc, cll_test, HR_test])
    else:
        with open('results_fully.csv', 'a', newline="\n") as f:
            writer = csv.writer(f)
            writer.writerow([args.dataset[5:]])
            writer.writerow([precision_test, recall_test, acc_val, ap_test, test_auc, cll_test, HR_test])
...
if args.dataset.startswith('ogbl'):
    ...
    def datasetsSnapShot(dataset_name):
        ...
        x = np.load("./datasets_LLGF/"+args.dataset+"_x.npy")
...
for run in range(args.runs):
    ...
    if args.only_test:
        ...
        if "cora" in args.dataset:
            checkpoint = torch.load('modelLLGF_cora_new_semi_ind.pth')
        elif "ACM" in args.dataset:
            checkpoint = torch.load('modelLLGF_ACM_new_semi_ind.pth')       
        elif "IMDB" in args.dataset:
            checkpoint = torch.load('modelLLGF_IMDB_new_semi_ind.pth')       
        elif "citeseer" in args.dataset:
            checkpoint = torch.load('modelLLGF_citeseer_new_semi_ind.pth')
        elif "photos" in args.dataset:
            checkpoint = torch.load('modelLLGF_photos_new_semi_ind.pth')
        elif "computers" in args.dataset:
            checkpoint = torch.load('modelLLGF_computers_new_semi_ind.pth')
