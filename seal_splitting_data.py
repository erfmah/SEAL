...
def train():
    ...
    for data in pbar:
        ...
        # test_auc = roc_auc_score(test_true.detach().numpy(), test_pred.detach().numpy())
...
@torch.no_grad()
def test(val_pred_total, val_true_total, test_pred_total, test_true_total):
    model.eval()
    ...
    val_pred_total = torch.cat((val_pred, val_pred_total))
    val_true_total = torch.cat((val_true, val_true_total))
    test_pred_total = torch.cat((test_pred, test_pred_total))
    test_true_total = torch.cat((test_true, test_true_total))
    ...
    return results, val_pred_total, val_true_total, test_pred_total, test_true_total
...
if args.dataset.startswith('ogbl'):
    ...
    def datasetsSnapShot(dataset_name):
        train_pos = np.load(dataset_name + "_train_pos.npy")
        val_pos = np.load(dataset_name + "_val_pos.npy")
        test_pos = np.load(dataset_name + "_test_pos.npy")
        val_neg = np.load(dataset_name + "_val_neg.npy")
        test_neg = np.load(dataset_name + "_test_neg.npy")
        x = np.load(dataset_name + "_x.npy")
...
#code for splitted data
val_pred_total = torch.tensor([])
val_true_total = torch.tensor([])
test_pred_total = torch.tensor([])
test_true_total = torch.tensor([])

if args.dataset.startswith('LLGF'):
    path = "datasets_LLGF_r/" + '_seal{}'.format(args.data_appendix) + str(0)
else:
...
for sub_list in range(100):
    ...
    if not sub_list == 0:
        if args.dataset.startswith('LLGF'):
            shutil.rmtree(path)
            path = "datasets_LLGF_r/" + '_seal{}'.format(args.data_appendix)+str(sub_list)
        else:
            path = dataset.root + '_seal{}'.format(args.data_appendix)
        use_coalesce = True if args.dataset == 'ogbl-collab' else False
        if not args.dynamic_train and not args.dynamic_val and not args.dynamic_test:
        ...
        print("############# checking "+str(sub_list+1)+"/100 of data #############")
        if not sub_list == 0:
            dataset_class = 'SEALDynamicDataset' if args.dynamic_train else 'SEALDataset'
            train_dataset = eval(dataset_class)(
                sub_list,
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        if epoch % args.eval_steps == 0:
            results, val_pred_total, val_true_total, test_pred_total, test_true_total = test(val_pred_total, val_true_total, test_pred_total, test_true_total)
            for key, result in results.items():
